...
const NoteList: React.FC = () => {
  ...
  const isRecentlyDeleted = folder?.name === 'ìµœê·¼ ?? œ????ª©';
  ...
  const handleDeleteNote = (e: React.MouseEvent, noteId: string) => {
    ...
    if (isRecentlyDeleted) {
      if (window.confirm('??ë©”ëª¨ë¥??êµ¬?ìœ¼ë¡??? œ?˜ì‹œê² ìŠµ?ˆê¹Œ?')) {
    ...
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          >
            <option value="modifiedAt">??ìµœê·¼ ?˜ì •??/option>
            <option value="createdAt">?“… ?ì„± ? ì§œ??/option>
            ...
            >
              <div className="p-5">
                ...
                <p className="text-gray-600 text-sm line-clamp-3 mb-4 min-h-[60px]">
                  {getPreviewText(note.content) || '?´ìš© ?†ìŒ'}
                </p>
